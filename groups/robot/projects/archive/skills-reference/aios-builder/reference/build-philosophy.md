...
**Low complexity** — Single file, clear requirement, obvious pattern. Just build it. No workpaper, no elicitation, self-check only.

**Medium complexity** — New file, new pattern, or multi-file change with a clear spec. Focused questions if anything is ambiguous. Workpaper required. Adversarial review on spec and deliverables.

**High complexity** — New agent, new skill, multi-component system, or architectural decision. Full elicitation to draw out domain knowledge. Detailed workpaper as source of truth. Adversarial review on spec and deliverables with full resolution.

**Signals that raise complexity:** multiple components, system-level scope, uncertainty about approach, multi-layer impact, domain knowledge required from the user.
...
**Collaborative (human present):** Elicit knowledge through targeted questions. Draft based on answers and codebase evidence. Review together. The builder structures what the user already knows — it doesn't invent from assumptions.
