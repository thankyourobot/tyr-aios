  ...
  while ((startIdx = parseState.buffer.indexOf(OUTPUT_START_MARKER)) !== -1) {
    ...
    try {
      ...
      if (parsed.schemaVersion === undefined) {
        logger.warn(
          { group: groupName },
          'Container output missing schemaVersion — possible stale agent-runner image',
        );
      } else if (parsed.schemaVersion !== 1) {
        logger.warn(
          { group: groupName, schemaVersion: parsed.schemaVersion },
          'Unexpected container output schemaVersion',
        );
      }
