...
Channels auto-enable when their credentials are present — no extra configuration needed. The Slack tokens stay in the host `.env`; NanoClaw reads them on the host process and posts to Slack on the agent's behalf, so agent containers never see them.
