...
Channels auto-enable when their credentials are present — no extra configuration needed. The Telegram token stays in the host `.env`; NanoClaw reads it on the host process and posts to Telegram on the agent's behalf, so agent containers never see it.
