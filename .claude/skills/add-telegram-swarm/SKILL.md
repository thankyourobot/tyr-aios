...
If using launchd, also add `TELEGRAM_BOT_POOL` to the launchd plist (`~/Library/LaunchAgents/com.nanoclaw.plist`) in the `EnvironmentVariables` dict — launchd doesn't auto-read `.env`.
...
7. Remove `TELEGRAM_BOT_POOL` from `.env` and launchd plist (if using launchd)
